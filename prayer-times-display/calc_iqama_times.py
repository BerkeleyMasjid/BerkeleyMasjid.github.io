...
def rough_iqama_time_estimation(df: pd.DataFrame, iqama_name, lower_bound_name, 
    lower_bound_offset, upper_bound_name, upper_bound_offset, iqama_step=0.5):

    iqama = 0
    for i in df.index:
        if iqama < df.loc[i, lower_bound_name] + lower_bound_offset:
            iqama = find_next_time_step(df.loc[i, lower_bound_name] + lower_bound_offset, incr_hr=iqama_step)
        elif iqama > df.loc[i, upper_bound_name] + upper_bound_offset:
            iqama = find_next_time_step(df.loc[i, upper_bound_name] + upper_bound_offset, reverse=True, incr_hr=iqama_step)
        df.loc[i, iqama_name] = iqama

    return df

def smooth_iqama_transitions(df: pd.DataFrame, iqama_name, lower_bound_name, 
    lower_bound_offset, upper_bound_name, upper_bound_offset, walkback=True):
        ...
        if last_val == df.loc[i, iqama_name]:
            ...
            if running_cnt > 21:
                ...
                if i-running_cnt-1 > 0:
                    ...
                    if (before_last_val > df.loc[i-running_cnt:i-1, lower_bound_name] + lower_bound_offset).all() and (
                            before_last_val < df.loc[i-running_cnt:i-1, upper_bound_name] + upper_bound_offset
                        ).all():
                        # print("3.2.1.1")
                ...
                if (next_val > df.loc[i-running_cnt:i-1, lower_bound_name] + lower_bound_offset).all() and (
                        next_val < df.loc[i-running_cnt:i-1, upper_bound_name] + upper_bound_offset
                    ).all():
                    # print("3.2.2")
                ...
                if walkback:
                    ...
                    last_transitions = find_last_n_transitions(df.loc[:,iqama_name], j, 2)
                    if len(last_transitions) != 2:
                        last_transitions = (0, 0)
                    half_way = j - (j - last_transitions[1])//2
                    walkback_success = False
                    while j >= half_way:
                        if new_iqama_value > df.loc[j, lower_bound_name] + lower_bound_offset and (
                                new_iqama_value < df.loc[j, upper_bound_name] + upper_bound_offset
                            ):
...
if __name__ == "__main__":
    ...
    fajr_transitions = [-1] + find_last_n_transitions(df.loc[:,"fajr_iqama"], df.index[-1], len(df.index))[::-1] + [df.index[-1]]
    ...
    for i, transition in enumerate(fajr_transitions):
        ...
        print(2)
        max_lower_bound = df.loc[fajr_transitions[i-1]+1:fajr_transitions[i], lower_bound_name].max() + lower_bound_offset
        min_upper_bound = df.loc[fajr_transitions[i-1]+1:fajr_transitions[i], upper_bound_name].min() + upper_bound_offset
        print(f"{iqama=}")
        ...
        if iqama < max_lower_bound:
            ...
            print("B")
            iqama = find_next_time_step(min_upper_bound, reverse=True, incr_hr=iqama_step)
    ...
    with open("timetable.json", "w+") as f:
        ...
        for i in df.index:
            timetable[original_index[i]] = {
                "fajr": [fraction_to_hours_minutes(df.loc[i, "fajr"]),fraction_to_hours_minutes(df.loc[i, "fajr_iqama"])],
                "sunrise": [fraction_to_hours_minutes(df.loc[i, "sunrise"]),""],
                "dhuhr": [fraction_to_hours_minutes(df.loc[i, "dhuhr"]), fraction_to_hours_minutes(df.loc[i, "dhuhr_iqama"])],
                "asr": [fraction_to_hours_minutes(df.loc[i, "asr"]),fraction_to_hours_minutes(df.loc[i, "asr_iqama"])],
                "maghrib": [fraction_to_hours_minutes(df.loc[i, "maghrib"]), "+10 min"],
                "isha": [fraction_to_hours_minutes(df.loc[i, "isha"]),fraction_to_hours_minutes(df.loc[i, "isha_iqama"])],
            }
